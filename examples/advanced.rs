...
// For convienience, you can also define prefixed base units directly.
...
// If a multiple of a unit is not given, you can provide it
// With Mile defined, this is the same: unit!(compound: MilePerHour, "mph", [(Mile, P1), (Hour, N1)]);
unit!(compound: MilePerHour, "mph", [(1.60934, Kilometre, P1), (Hour, N1)]);

fn main() {
    // Working with those units works just like the other examples
    let length = Foot::new(500.0);
    let time = Minute::new(2.0);

    let speed = length / time;
    println!(
        "{:.2} in {:.2} equal a speed of {:.2}",
        length,
        time,
